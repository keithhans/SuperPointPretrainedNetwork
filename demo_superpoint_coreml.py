...
class SuperPointFrontend(object):
  ...
  def __init__(self, weights_path, nms_dist, conf_thresh, nn_thresh, width, height,
               cuda=False):
    ...
    onnx_model_name = "superpoint_" + str(width) + "_" + str(height) + ".onnx" 
    ...
    model = convert(model=onnx_model_name, minimum_ios_deployment_target='13')
    # Save the CoreML model
    coreml_model_name = "superpoint_" + str(width) + "_" + str(height) + ".mlmodel" 
    model.save(coreml_model_name)

    # Load the network in inference mode.
    self.mlmodel = coremltools.models.MLModel(coreml_model_name)

    # try quantization
    #self.mlmodel = quantization_utils.quantize_weights(model, nbits=8, quantization_mode="kmeans")
    #self.mlmodel.save('int8' + coreml_model_name)
  ...
  def run(self, img):
    ...
    #outs = self.session.run([self.session.get_outputs()[0].name, self.session.get_outputs()[1].name], 
    #  {self.session.get_inputs()[0].name: inp})
    
    outs = self.mlmodel.predict({'in': inp})
